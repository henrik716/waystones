...
export function generateNotebook(model: DataModel, source: SourceConnection): string {
  ...
  cells.push(
    ...
    code(
      '# Two separate calls, not one — worker-stac and stac-sync have no compose-level',
      '# dependency on each other (so a partitioned override further down can bring up',
      '# stac-sync afterward without also re-triggering worker-stac with default settings).',
      '# run_compose() blocks until each finishes, so this still guarantees worker-stac has',
      '# actually produced something in MinIO before stac-sync tries to sync it out.',
      'run_compose("up", "worker-stac")',
      'run_compose("up", "stac-sync")',
      '',
