...
<style>
  ...
  .col-cta { flex-shrink: 0; display: flex; flex-direction: column; justify-content: center; align-items: flex-end; padding: 0 1.3rem; gap: 0.4rem; }
  .col-cta a.main-cta { font-size: 0.85rem; font-weight: 700; color: var(--brand); white-space: nowrap; text-decoration: none; }
  .col-cta a.main-cta:hover { text-decoration: underline; }
  .col-cta a.sec-cta { font-size: 0.72rem; font-weight: 600; color: #64748b; white-space: nowrap; text-decoration: none; }
  .col-cta a.sec-cta:hover { color: var(--brand); text-decoration: underline; }
...
<div class="col-row">
  ...
  <div class="col-body">
    <div>
      <a class="col-title" href="{{ config.server.url }}/collections/{{ col.id }}/items">{{ col.title | default(col.id) }}</a>
    </div>
  ...
  ${crsMetaBlock}
  <div class="col-cta">
    <a href="{{ config.server.url }}/collections/{{ col.id }}/items" class="main-cta">Explore Data &rarr;</a>
    <a href="{{ config.server.url }}/collections/{{ col.id }}" class="sec-cta">Metadata</a>
  </div>
</div>
{% endfor %}
{% endblock %}
